"""ST-VQA-style OCR subset adapter with evidence-removal pair building."""
...
class STVQADatasetAdapter(TextVQADatasetAdapter):
    """Adapter for ST-VQA-style OCR tasks with removable evidence."""

    def _build_mock_subset(self, config: DatasetConfig, output_root: Path) -> list[dict]:
        image_dir = output_root / "images" / "stvqa_mock"
        image_dir.mkdir(parents=True, exist_ok=True)
        rows = [
            ("SALE", "What word is written on the banner?"),
            ("12", "What number appears on the sign?"),
            ("PARKING", "What word is shown on the board?"),
        ]
        subset: list[dict] = []
        for idx, (text, question) in enumerate(rows):
            sample_id = f"stvqa_mock_{idx:03d}"
            answer = text.lower()
    ...
    def build_pairs(
        ...
        image_dir = output_root / "images" / "stvqa_pairs"
        image_dir.mkdir(parents=True, exist_ok=True)
        samples: list[PairSample] = []

        for idx, row in enumerate(records):
            pair_id = f"stvqa_pair_{idx:04d}"
