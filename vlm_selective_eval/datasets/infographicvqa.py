"""InfographicVQA-style OCR subset adapter with evidence-removal pair building."""
...
class InfographicVQADatasetAdapter(TextVQADatasetAdapter):
    """Adapter for InfographicVQA-style OCR tasks with removable evidence."""

    def _build_mock_subset(self, config: DatasetConfig, output_root: Path) -> list[dict]:
        image_dir = output_root / "images" / "infographicvqa_mock"
        image_dir.mkdir(parents=True, exist_ok=True)
        rows = [
            ("42%", "What percentage is highlighted?"),
            ("EUROPE", "Which region label is shown?"),
            ("2019", "What year appears in the chart?"),
        ]
        subset: list[dict] = []
        for idx, (text, question) in enumerate(rows):
            sample_id = f"infographicvqa_mock_{idx:03d}"
            answer = text.lower()
    ...
    def build_pairs(
        ...
        image_dir = output_root / "images" / "infographicvqa_pairs"
        image_dir.mkdir(parents=True, exist_ok=True)
        samples: list[PairSample] = []

        for idx, row in enumerate(records):
            pair_id = f"infographicvqa_pair_{idx:04d}"
