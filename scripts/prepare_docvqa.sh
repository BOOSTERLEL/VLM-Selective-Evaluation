...
ROOT="${1:-/home/featurize/data/docvqa}"
VAL_JSON_SRC="${2:-}"
IMAGES_SRC="${3:-}"
OCR_SRC="${4:-}"
...
if [[ -z "${VAL_JSON_SRC}" || -z "${IMAGES_SRC}" || -z "${OCR_SRC}" ]]; then
  cat <<'EOF'
DocVQA download is not a public direct-link dataset like TextVQA.

Official pages:
- https://www.docvqa.org/datasets/docvqa
- https://site.docvqa.org/datasets

Please download or copy these validation-split assets first:
1. DocVQA validation annotation JSON
2. DocVQA images archive or directory
3. DocVQA OCR archive or directory

Then rerun, for example:
  bash prepare_docvqa.sh /home/featurize/data/docvqa \
...
python scripts/build_docvqa_source.py \
  --annotations "${ROOT}/val_v1.0.json" \
  --ocr-dir "${ROOT}/ocr" \
  --image-dir "${ROOT}/images" \
  --output "${ROOT}/docvqa_val_source.json"
